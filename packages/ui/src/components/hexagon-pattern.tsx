...
function edgeLexKey(a: HexPoint, b: HexPoint): string {
  ...
  return `${p[0].toFixed(4)},${p[1].toFixed(4)}|${q[0].toFixed(4)},${q[1].toFixed(4)}`
}
...
function getHexSpacing(
  ...
  if (direction === "horizontal") {
    const colStep = round((3 * r) / 2 + (sqrt3 * gap) / 2)
    const rowStep = round(sqrt3 * r + gap)

    return {
      colStep,
      rowStep,
      tileW: round(colStep * 2),
      tileH: rowStep,
    }
  }

  const colStep = round(sqrt3 * r + gap)
  const rowStep = round((3 * r) / 2 + (sqrt3 * gap) / 2)

  return {
    colStep,
    rowStep,
    tileW: colStep,
    tileH: round(rowStep * 2),
  }
}
...
function getTileGeometry(
  ...
  if (direction === "horizontal") {
    ...
    const canonical: [number, number][] = [
      [round(colStep / 2), round(rowStep / 2)],
      [round((colStep * 3) / 2), rowStep],
    ]

    const centers: [number, number][] = []
    for (const [cx, cy] of canonical) {
      centers.push([cx, cy])
      if (cy - r < 0) centers.push([cx, round(cy + tileH)])
      if (cy + r > tileH) centers.push([cx, round(cy - tileH)])
      if (cx - r < 0) centers.push([round(cx + tileW), cy])
      if (cx + r > tileW) centers.push([round(cx - tileW), cy])
      if (cy - r < 0 && cx - r < 0) centers.push([round(cx + tileW), round(cy + tileH)])
      if (cy - r < 0 && cx + r > tileW) centers.push([round(cx - tileW), round(cy + tileH)])
      if (cy + r > tileH && cx - r < 0) centers.push([round(cx + tileW), round(cy - tileH)])
      if (cy + r > tileH && cx + r > tileW)
        centers.push([round(cx - tileW), round(cy - tileH)])
    }
...
function hexCenter(
  ...
  if (direction === "horizontal") {
    ...
    const x = round(col * colStep + colStep / 2)
    const y = round(row * rowStep + rowStep / 2 + (col % 2 !== 0 ? rowStep / 2 : 0))
    return [x, y]
  } else {
    const { colStep, rowStep } = getHexSpacing(r, direction, gap)
    const x = round(col * colStep + colStep / 2 + (row % 2 !== 0 ? colStep / 2 : 0))
    const y = round(row * rowStep + rowStep / 2)
    return [x, y]
  }
}
